...
sppc.getInvestValues = function() {
	...
	if((isNaN(sppc.invPanels))    ||
	   (isNaN(sppc.invInverter))  || 
	   (isNaN(sppc.invInstall))   ||
	   (sppc.invPanels == 0)      ||
	   (sppc.invInverter == 0)    || 
	   (sppc.invInstall == 0))
		return false;
...
sppc.getMaintValues = function() {
	...
	if((isNaN(sppc.maintInverter)) ||
	   (isNaN(sppc.maintOther))    || 
	   (sppc.maintInverter == 0)   ||
	   (sppc.maintOther == 0))
		return false;
...
sppc.calcCosts = function() {
	...
	if(!(sppc.getInvestValues())) return;
	if(!(sppc.getMaintValues())) return;
...
sppc.getFincValues = function() {
	...
	if((isNaN(sppc.fincPercent))    ||
	   (isNaN(sppc.fincTime)) 		|| 
	   (isNaN(sppc.fincInterest))   ||
	   (sppc.fincPercent == 0)      ||
	   (sppc.fincTime == 0)   		|| 
	   (sppc.fincInterest == 0))
		return false;
...
sppc.calcFincCosts = function() {
	...
	if(!(sppc.getFincValues())) return;

	sppc.fincTotal = sppc.fincTime * sppc.costsYearly[0] * sppc.fincPercent * sppc.fincInterest;
...
sppc.getStrgValues = function() {
	...
	if((isNaN(sppc.strgCost))     ||
	   (isNaN(sppc.strgLifetime)) || 
	   (isNaN(sppc.strgEff))      ||
	   (isNaN(sppc.strgShare))    ||
	   (sppc.strgCost     == 0)   ||
	   (sppc.strgLifetime == 0)   || 
	   (sppc.strgEff      == 0)   ||
	   (sppc.strgShare    == 0))
...
sppc.calculate = function() {
	...
	chart += Math.round(sppc.costsComul[i] / (sppc.energyComul[30] / 1000) * 1000) / 1000 + "]";

	tot = sppc.invPanels + sppc.invInverter + sppc.strgCost + 
		  sppc.invInstall + sppc.mainTotal + sppc.fincTotal;

	totPanels = Math.round(sppc.invPanels   / tot * 100); 
	totInvert = Math.round(sppc.invInverter / tot * 100);
	totStorag = Math.round(sppc.strgCost    / tot * 100);
	totInstal = Math.round(sppc.invInstall  / tot * 100);
	totMainte = Math.round(sppc.mainTotal   / tot * 100);
	totFinanc = Math.round(sppc.fincTotal   / tot * 100);

	totals = "<br><br>";
	totals += "Panels: " + sppc.invPanels + " €/kWp " + totPanels + " %<br>";
	totals += "Inverter: " + sppc.invInverter + " €/kWp " + totInvert + " %<br>";
	totals += "Storage: " + sppc.strgCost + " €/kWp " + totStorag + " %<br>";
	totals += "Installation: " + sppc.invInstall + " €/kWp " + totInstal + " %<br>";
	totals += "Maintenance: " + sppc.mainTotal + " €/kWp " + totMainte + " %<br>";
	totals += "Financing: " + sppc.fincTotal + " €/kWp " + totFinanc + " %<br>";
