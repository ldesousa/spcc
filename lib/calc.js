...
spcc.calcCosts = function() {
	...
	{
		spcc.costsYearly[j] += spcc.invInverter * spcc.sysCap;
		spcc.mainTotal += spcc.invInverter * spcc.sysCap; 
	}
...
spcc.calculate = function() {
	...
	chartCategories = new Array(spcc.sysLifetime - 10);
	if(spcc.DEBUG) chartData = "<br><br>[";

	for(i = 10; i <= spcc.sysLifetime; i++)
	{
		chartSeries[i-10] = completeSeries[i];
		chartCategories[i-10] = (i).toString();
		if((spcc.DEBUG) && (i != spcc.sysLifetime))
			chartData += Math.round(spcc.costsComul[i] / (spcc.energyComul[i] / 1000) * 1000) / 1000 + ", ";
	}

	if(spcc.DEBUG)
		chartData += Math.round(spcc.costsComul[i] / (spcc.energyComul[spcc.sysLifetime] / 1000) * 1000) / 1000 + "]";
	
	/*tot = (spcc.invPanels + spcc.invInverter + spcc.invInstall) * spcc.sysCap + 
		  spcc.strgCost + spcc.mainTotal + spcc.fincTotal;*/
	tot = spcc.costsComul[spcc.sysLifetime];

	totPanels = Math.round(spcc.invPanels   * spcc.sysCap / tot * 100); 
	totInvert = Math.round(spcc.invInverter * spcc.sysCap / tot * 100);
	totInstal = Math.round(spcc.invInstall  * spcc.sysCap / tot * 100);
	...
	spcc.charts.renderCostTime('graph-container', chartSeries, chartCategories);
